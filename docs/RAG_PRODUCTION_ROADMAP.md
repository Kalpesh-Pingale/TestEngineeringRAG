...
> isolated per-project storage) and 5 (per-project Jira credentials, config YAML)
> are still open — needed for the 30+ project / cross-tenant scale this phase
> targets, not for a handful of projects on one workspace.
